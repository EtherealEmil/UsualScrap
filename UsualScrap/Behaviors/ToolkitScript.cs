...
{
    public class ToolkitScript : GrabbableObject
    {
        ...
        AudioSource[] toolkitAudio;
        AudioSource toolkitSound;
        AudioClip Smash;
        ...
        public void Awake()
        {
            try
            {
                toolkitSound = this.transform.Find("ToolkitSounds").gameObject.GetComponent<AudioSource>();
                toolkitAudio = this.transform.Find("ToolkitSounds").gameObject.GetComponents<AudioSource>();
                Smash = toolkitAudio[0].clip;
                sparkEffect = this.transform.Find("SparkEffect").GetComponent<ParticleSystem>();
            }
            catch
            {
                if (toolkitAudio == null)
                {
                    print("ERROR: Toolkit audio not found.");
                }
                if (sparkEffect == null)
                {
                    print("ERROR: Toolkit effect not found.");
                }
            }
        }
            ...
            if (buttonDown) {
                ...
                if (detectedtrapsArray.Length > 0)
                {
                    foreach (Collider collider in detectedtrapsArray)
                    {
                        ...
                        {
                            //print("US - Toolkit initializing trap!");
        ...
        private System.Collections.IEnumerator DismantleTrap(string trapType)
        {
            ...
            { 
                ...
                sparkEffect.Play();
                toolkitSound.pitch = Random.Range(.4f, .6f);
                toolkitSound.PlayOneShot(Smash);
